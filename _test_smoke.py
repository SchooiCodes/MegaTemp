...
def _test_fs():
	...
	_check(
		w is None or isinstance(w, Config),
		"write_default_config returns Config or None",
	)
	...
	_check(
		len(creds_list) >= 2,
		f"list_credentials has {len(creds_list)} entries (expected >= 2)",
	)
...
def _test_etc():
	...
	_check(VERSION == "v1.5.0", f"VERSION is {VERSION}")
	...
	with _timed("checkpoint save/load"):
		...
		cp = load_checkpoint()
	_check(
		cp is not None and cp.completed == 5 and cp.failed == 2, "checkpoint round-trip"
	)
	clear_checkpoint()
	cp2 = load_checkpoint()
...
def _test_main():
	...
	_check(
		"--proxy-server=http://user:pass@5.6.7.8:3128" in args3,
		"browser args uses proxy manager",
	)
	m._proxy_manager = ProxyManager()

	_check(
		m._HARMLESS_ASYNC_ERRORS == ("Target closed", "No session with given id"),
		"harmless errors defined",
	)
	_check(
		len(m._BROWSER_BASE_ARGS) >= 10,
		f"browser base args count: {len(m._BROWSER_BASE_ARGS)}",
	)
	...
	_check(counter[0] == 1, "retry called once (not retried)")
	...
	from pymailtm.pymailtm import (
		CouldNotGetAccountException,
		CouldNotGetMessagesException,
	)

	_check(
		issubclass(CouldNotGetAccountException, Exception),
		"CouldNotGetAccountException is Exception",
	)
	_check(
		issubclass(CouldNotGetMessagesException, Exception),
		"CouldNotGetMessagesException is Exception",
	)
...
def _test_password_generator():
	...
	for length in [8, 16, 32]:
		...
		_check(any(c.isdigit() for c in pw), "password has digits")
		_check(any(not c.isalnum() for c in pw), "password has special chars")
...
def _test_cli():
	...
	with _timed("CLI --help"):
		r = subprocess.run(
			base + ["--help"], capture_output=True, text=True, timeout=15
		)
	...
	with _timed("CLI --version"):
		r = subprocess.run(
			base + ["--version"], capture_output=True, text=True, timeout=15
		)
	_check(r.returncode == 0, "--version exits 0")
	_check("v1.5.0" in r.stdout, f"--version shows {r.stdout!r}")

	with _timed("CLI --health --json"):
		r = subprocess.run(
			base + ["--health", "--json"], capture_output=True, text=True, timeout=30
		)
	...
	with _timed("CLI --health"):
		r = subprocess.run(
			base + ["--health"], capture_output=True, text=True, timeout=30
		)
	_check(r.returncode == 0, "--health exits 0")

	with _timed("CLI --quiet --version"):
		r = subprocess.run(
			base + ["--quiet", "--version"], capture_output=True, text=True, timeout=15
		)
	_check(r.returncode == 0, "--quiet --version exits 0")

	with _timed("CLI --webhook-url http://x.com --version"):
		r = subprocess.run(
			base + ["--webhook-url", "http://example.com/hook", "--version"],
			capture_output=True,
			text=True,
			timeout=15,
		)
...
def main():
	...
	if _timings:
		slow = [(label, t) for label, t in _timings if t > 0.2]
		if slow:
